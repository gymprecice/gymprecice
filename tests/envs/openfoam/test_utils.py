...
from gymprecice.envs.openfoam.utils import (get_patch_geometry, get_interface_patches, read_line)
...
def test_get_patch_geometry(test_case):
    ...
    assert all([np.allclose(out, expect) for out, expect in zip(output['inlet'].values(), expected['inlet'].values())])
