from adapters.gymprecice import Adapter
...
from utils.foamutil import _get_interface_patches, _get_patch_geometry, _make_env_dir


class TurekHronFSIEnv(Adapter):
    ...
    def __init__(self, options, idx=0) -> None:
        ...
        _make_env_dir(self.env_dir, self._solver_list)

        self.cylinder_origin = np.array([0.2, 0.2, 0.1])
        ...
        self._steps_per_action = 1
        self._start_control_t = 0.0
        ...
        self._reward_info = {
            ...
            'Cd_column': 1,
            'Cl_column': 3,
            'dumped_file_path': '/postProcessing/forceCoeffs/0/coefficient.dat',
            'file_handler': None,
            'data': None
        }
        ...
        super(TurekHronFSIEnv, self).__init__(options)
    
    def step(self, action):
        if self._t < self._start_control_t:
            action = 0 * action 
    ...
    def _action_to_patch_field(self, action):
        ...
        for idx, patch_name in enumerate(self.actuator_geometric_data.keys()):
            patch_ctr = np.array([radius * np.cos(theta0[idx]) + origin[0], radius * np.sin(theta0[idx]) + origin[1], origin[2]])
    ...
    def _probes_to_observation(self):
        if (self._time_window == 0):
            return self._observation_info['initial_observation']
        ...
        latest_time_data = np.array(probes_data[-2][2])  # only the last timestep and remove the time and size columns
    ...
    def _forces_to_reward(self):
        ...
        forces_data = self._reward_info['data'][:-1]
    ...
    def _read_probes_from_file(self):
        ...
        data_path = f"{self._openfoam_solver_path}{self._observation_info['dumped_file_path']}"
        ...
        time_stamp = 0
        while not math.isclose(time_stamp, self._t):  # read till the end of a time-window   
            while True:
        ...
        assert math.isclose(time_stamp, self._t), f"Error: mismatched time data: {time_stamp} vs {self._t}"  

    def _read_forces_from_file(self):
        # sequential read of a single line (last line) of forces file at each RL step
        data_path = f"{self._openfoam_solver_path}{self._reward_info['dumped_file_path']}"
        ...
        while not math.isclose(time_stamp, self._t):  # read till the end of a time-window         
            while True:
                ...
                if not is_comment and n_forces == self._reward_info['n_forces']:
                    break
            if not self._reward_info['data']:
                self._reward_info['data'].append([time_stamp, n_forces, forces_data])
            else:
                if self._reward_info['data'][-1][0] == time_stamp:
                    self._reward_info['data'][-1] = [time_stamp, n_forces, forces_data]
                else:
                    self._reward_info['data'].append([time_stamp, n_forces, forces_data])
        assert math.isclose(time_stamp, self._t), f"Error: mismatched time data: {time_stamp} vs {self._t}"  
    ...
    def _smooth_step(self, action):
        ...
        while subcycle < self._steps_per_action:
            ...
            if isinstance(smoothed_action ,np.ndarray):
                next_obs, reward, done, info =  super(TurekHronFSIEnv, self).step(smoothed_action)
            else:
                next_obs, reward, done, info =  super(TurekHronFSIEnv, self).step(smoothed_action.cpu().numpy())
