import gym
from envs.fsi_envs import TurekHronFSIEnv 
...
from utils.constants import (EPSILON, LOG_EPSILON, BASE_PATH)
from utils.oldutil import set_training_dir
from utils.envutil import AsyncVectorEnv
...
if __name__ == '__main__':
    ...
    options = {
        'solvers':
        {
            'name_list': ["fluid-openfoam", "solid-dealii"],
            'base_path': f"{BASE_PATH}/examples/turek_hron_fsi3",
    ...
    if args.track:
        import wandb
        run_name = f'FSI_PPO_{int(time.time())}'
    ...
    def make_env(options, idx, wrappers=None):
        def _make_env():
            env = TurekHronFSIEnv(options, idx)
