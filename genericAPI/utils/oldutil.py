...
def get_mesh_data(foldername, filename):
    xml_string = load_file(foldername, filename)
    ...
    # we only have one RL participant
    for item_ in xml_tree['precice-configuration']['solver-interface']['participant']:
        if 'rl' in item_['@name'].lower():
            RL_participant = copy.deepcopy(item_)
            if verbose_mode: 
                print(RL_participant)
            break

    RL_paricipant = {}
    if 'read-data' in RL_participant.keys():
        if isinstance(RL_participant['read-data'], dict):
            RL_participant['read-data'] = [RL_participant['read-data']]
        for item_ in RL_participant['read-data']:
            if item_['@mesh'] not in RL_paricipant.keys():
                RL_paricipant[item_['@mesh']] = {"read": [], "write": []}
            RL_paricipant[item_['@mesh']]["read"].append(item_['@name'])

    if 'write-data' in RL_participant.keys():
        if isinstance(RL_participant['write-data'], dict):
            RL_participant['write-data'] = [RL_participant['write-data']]

        for item_ in RL_participant['write-data']:
            if item_['@mesh'] not in RL_paricipant.keys():
                RL_paricipant[item_['@mesh']] = {"read": [], "write": []}
            RL_paricipant[item_['@mesh']]["write"].append(item_['@name'])

    return scaler_variables, vector_variables, mesh_list, RL_paricipant
...
def mod_mapping_lists(mapping_list, solver_indices):
    ...
    for mapping_item in mapping_list:
        for idx in solver_indices:
            ...
            # if 'rl' in mapping_item['@from'].lower():
            #     new_mapping_list.append({
            #         '@direction': mapping_item['@direction'],
            ...
            #         '@constraint': mapping_item['@constraint'],
            #     })
            # elif 'rl' in mapping_item['@to'].lower():
            #     new_mapping_list.append({
            #         '@direction': mapping_item['@direction'],
            #         '@from': mapping_item['@from'] + f'_{idx}',
            #         '@to': mapping_item['@to'],
...
def repeat_coupling(coupling_dict, n_parallel_env):
    ...
    for solver_idx in range(n_parallel_env):
        ...
        if 'rl' not in participants['@first'].lower():
            participants['@first'] = participants['@first'] + f'_{solver_idx}'
        elif 'rl' not in participants['@second'].lower():
            participants['@second'] = participants['@second'] + f'_{solver_idx}'
        else:
            raise Exception('parallel parsing: rl have to be either the first or second')
    ...
    for key in mod_tree_sub.keys():
        if key == "@dimensions":
            ...
            for participant_item in participant_list:
                if 'rl' in participant_item['@name'].lower():
                    if verbose_mode: 
                        print("===========")
...
def get_actuation_patches_OF(path_to_precicedict):
    """ 
...
def wait_for_file(file_object, sleep_time=0.1):
    ...
    status = os.stat(file_object)
    last_st = status.st_ctime_ns
    while True:
        time.sleep(sleep_time)
        status = os.stat(file_object)
        current_st = status.st_ctime_ns
...
def get_episode_end_time(foldername, filename, coupling_method='parallel-explicit'):
    foam_string = load_file(foldername, filename)
    xml_tree = xmltodict.parse(foam_string, process_namespaces=False, dict_constructor=dict)
    coupling_method = 'coupling-scheme:' + coupling_method
    solver_interface = xml_tree['precice-configuration']['solver-interface']

    if coupling_method in solver_interface.keys():
...
if __name__ == '__main__':
    ...
    parallel_folders_list = [f'/data/ms2137/rl_play/examples/cylinder2D_openfoam/temp_{idx}' for idx in range(n_parallel_env)]
...
def set_training_dir(options):
    ...
    #shell_script_name = options['shell_script_name']
    precice_config_file_name = options['precice_config_file_name']
    case_dir= join(base_path, case_name)
    precice_config_file = join(base_path, precice_config_file_name)
    #shell_script_file = join(base_path, shell_script_name)
    ...
    try:
        os.system(f'cp -r {case_dir} {run_dir}')
    except Exception as e:
    ...
    # # copy shell script to run dir
    # try:
    #     os.system(f'cp {shell_script_file} {run_dir}')
    # except Exception as e:
    #     raise Exception(f'Failed to copy shell script file to run dir: {e}')
    ...
    for line in fin:
        ...
        if keyword in line and not replaced:
            new = f'{line.partition(keyword)[0]} {keyword}="{run_dir}" />'
            new = new + "\n" if not new.endswith("\n") else new  
            line = new
            replaced = True
